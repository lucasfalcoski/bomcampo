...
const INTENT_PATTERNS: Record<string, RegExp[]> = {
  high_risk_today: [
    /[ée]\s+seguro\s+(entrar|aplicar)/i,
  ...
  create_task: [
    /agenda\s+visita\s+agr[ôo]nomo\s+para/i,
    /preciso\s+agendar\s+visita\s+t[ée]cnico/i,
    /deixa\s+esquecer\s+comprar\s+novo/i,
    /lembra\s+verificar\s+n[íi]vel\s+reservat[óo]rio/i,
    /me\s+lembr(a|e)/i,
  ...
  observation_diagnosis: [
    ...
    /percevejo/i, /pulg[ãa]o/i, /trip(e|s)/i, /mosca\s+branca/i, /nematoide/i, /ant?racnose/i,
